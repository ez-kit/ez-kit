...
packages/
  ...
  valtio-kit/         # @ez-kit/valtio-kit — Valtio context store + URL search-params sync engine
  data-grid/
...
**`@ez-kit/valtio-kit`** — sibling to zu-store but Valtio-backed. `createContextStore(factory)` returns `{ Provider, useStore, useSnapshot, Item }` where `useStore()` is the raw mutable proxy (mutate directly, e.g. `state.count++`) and `useSnapshot()` is the readonly auto-tracked read. The `search-params` subsystem (subpath export `@ez-kit/valtio-kit/search-params`) is a URL-as-state sync engine: a binding `engine` syncs proxy state to query params via pluggable `codecs/` (`paramString`/`paramNumber`/`paramArray`/`paramEnum`/`paramJson`/…), `layouts/` (`flat`/`json`/`nested` key placement), router adapters (`routers/next`, `routers/react-router` — separate subpath exports), optional `validators/zod` and `encoders/qs`. Fields are declared via `withSearchParams`/`searchParam` decorators or the `withSearchParamsFields` accessor builder. Each integration is an optional peer dependency.
