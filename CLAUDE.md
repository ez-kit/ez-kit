...
**Documented option names are type-checked** — `apps/docs/test/docs-option-names.test.ts` (helpers in `apps/docs/test/docs-options/`) resolves every option name in the data-grid docs' markdown option tables against the **real** exported types, via `ts.TypeChecker.getPropertiesOfType()` on a `ts.Program` built from `apps/docs/tsconfig.json`. It guarantees that a fabricated option name on a mapped page fails CI instead of shipping silently — the failure prints file:line, the bogus name, the legal keys of the governing type, and a "did you mean". This deliberately is **not** a grep: `enableSorting`, `enableColumnFilters`, `enableRowSelection` and `manualPagination` all appear literally in `packages/data-grid/core/src/create-table.ts` (the core sets them as _internal_ TanStack options) while being illegal in the public config, so a substring check would bless exactly the defect class this test exists to catch. Because package exports resolve to `./dist`, the data-grid packages must be **built** before the test runs; the normal turbo `test` task (`dependsOn: ["^build"]`) already enforces that. Coverage is the explicit page → type map in `apps/docs/test/docs-options/page-type-map.ts` — currently the 19 pages hand-verified in the docs API audit, keyed by file path **plus the heading above each table** so multiple tables in one file map independently. To extend coverage: verify the page's tables against the real types by hand, add the path to `DocPage`, and add a `PAGE_ENTRIES` entry classifying **every** table on the page as either an `optionTables` entry (with the governing type) or a `nonOptionTables` entry (with a reason) — an unclassified table fails the test. Rows that intentionally document a non-key (e.g. the literal `false` a per-column slot accepts) go in `OPTION_EXCEPTIONS`, each with its reason.
