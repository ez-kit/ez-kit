 ...
 *
 * The defect was kit-specific in a way that is invisible from the shared layer — heroui's
 * `<Td>` is a block box, so `text-align` alone always worked there — which is exactly why the
 * spec is written once and the project runs it against every kit.
 */
...
test("align 'end' moves the text of body and footer cells, not just the header", async ({ page }) => {
...
test("align 'end' reaches the footer cell specifically", async ({ page }) => {
	...
	if (footer === undefined) throw new Error('the footer has no right-aligned cell to measure')
	expect(footer.rightGap).toBeLessThan(footer.leftGap)
})
