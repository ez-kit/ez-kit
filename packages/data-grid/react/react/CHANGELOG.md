...
- f41b16d: feat: `layout.classNames` for the grid shell's two boxes
...
- f41b16d: feat: the grid shell's wrapper and scrollport are slots, and `ref` declares the scroller
...
- f41b16d: fix: factory `defaults` now reach the inline `<DataGrid data columns />` too
...
- ed0250e: **Breaking:** the rest of the second API audit — exports, closed sets, and two either/or types.
...
- ed0250e: **Breaking:** bulk delete moves from `selection.panel` to `deleting.bulk`.
...
- fe267fa: `cellTypes` layers entry by entry instead of replacing whole cell types.
...
- fe267fa: Columns can say how they are aligned.
...
- fe267fa: **Breaking:** a column's `pinning` keys are `side` / `initialSide`, and the common case is a scalar.
...
- fe267fa: **Breaking:** `size` / `minSize` / `maxSize` on a column collapse into one `width`.
...
- ed0250e: `DataGridDefaultOptions` accepts `boolean` for `creating` / `editing` / `deleting`.
...
- ed0250e: **Breaking:** `DATA_GRID_DEFAULTS` is keyed by the option path it defaults.
...
- 1f19a95: Gate what leaves the grid while a query draft is pending (`deferredApply`).
...
- ed0250e: **Breaking:** deleting is a namespace on the table, like creating and editing.
...
- ed0250e: **Breaking:** the `@ez-kit/data-grid-react/cell-types` sub-export is gone. Everything it held —
...
- ed0250e: **Breaking:** validation timing has one spelling per level, and the per-column one moves under its feature.
...
- d825988: Let `DataGridOptionsProvider` and `createDataGrid({ defaultOptions })` describe writes.
...
- abaa123: **Breaking:** the page-size selector is now configured through `pagination.pageSizeOptions` instead of the separate `pageSizer` prop, and the `PageSizerConfig` type is gone.
...
- ed0250e: **Breaking:** the table-level column-hiding option is renamed `columnVisibility` → `visibility`, so
...
- ed0250e: **Breaking:** a custom row action's `icon` is a named glyph or your own element, never a bare string.
...
- ed0250e: **Breaking:** `selection.panel` → `selection.bar`, and one name for the action bar's render mode.
...
- ed0250e: `selection.multiple` is now `selection.multi`, and it finally does something.
...
- ed0250e: Align `SelectionPanelVariant` with the repo's closed-set convention and resolve `filtering.variant` to its documented default.
...
- ed0250e: Export the types every documented signature already names.
...
- ed0250e: A column's cell value is typed from its `accessorKey`.
...
- ed0250e: **Breaking:** `editing`, `creating`, `deleting` and `rowActions` join the `boolean | Config` row.
...
- ed0250e: Docs: the `editing/**` section and `cells/cell-types.mdx` documented an API that does not exist.
...
- af8d6a1: Fix pinned columns coming unstuck near the end of horizontal scroll.
...
- 1f068e5: Fix `globalFiltering.onChange` never firing, which silently disabled server-side global search.
...
- 1f068e5: Stop controlled-state syncing from updating subscribed components during render.
...
- 1f068e5: Fix a column that is both pinned and resizable losing its pinning.
...
- 904d9df: Clamp `pageIndex` to the last valid page when a manual-pagination `rowCount` shrinks under the user.
