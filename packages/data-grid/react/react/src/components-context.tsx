...
function DefaultToolbar({ children }: ToolbarProps) {
...
function DefaultPagination({
  ...
  return (
    <div>
      <button type="button" onClick={onFirstPage} disabled={!canPreviousPage}>
        {'«'}
      </button>
      ...
      <button type="button" onClick={onLastPage} disabled={!canNextPage}>
        {'»'}
      </button>
    </div>
  )
...
export function GridComponentsProvider({
  components,
  children,
}: GridComponentsProviderProps) {
  const merged: Required<GridComponents> = components
    ? { ...defaultComponents, ...components }
    : defaultComponents
  return (
    <GridComponentsContext value={merged}>{children}</GridComponentsContext>
  )
}
