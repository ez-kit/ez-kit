...
export function CreatingModal() {
  ...
  return (
    <ModalComponent open={isOpen} onClose={() => { table.cancelCreating() }} title="Create">
      <AutoForm mode="creating" />
      <button type="button" onClick={() => void table.commitCreating()}>
        Save
      </button>
      <button type="button" onClick={() => { table.cancelCreating() }}>
        Cancel
      </button>
    </ModalComponent>
  )
