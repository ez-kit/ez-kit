...
describe('buildActionItems', () => {
	it('namespaces ids so a custom action cannot collide with a built-in one', () => {
		const [item] = buildActionItems([{ id: 'edit', label: 'Edit copy', onSelect: () => {} }])
	...
	it('carries a consumer-supplied element through as the icon', () => {
		...
		const [item] = buildActionItems([{ id: 'send', label: 'Send invoice', icon: glyph, onSelect: () => {} }])
	...
	it('carries disabled, destructive and onSelect through', () => {
		const onSelect = () => {}
		const [item] = buildActionItems([{ id: 'purge', label: 'Purge', disabled: true, destructive: true, onSelect }])
