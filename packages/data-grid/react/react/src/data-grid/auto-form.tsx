  ...
  return (
    <div>
      {table.getAllColumns().map((col) => {
        ...
        if (customComp) {
          return <div key={col.id}>{customComp({ value, onChange, ...(meta?.config !== undefined ? { config: meta.config } : {}) })}</div>
        }

        // 2. registry by cellType
        if (meta?.cellType) {
          const regComp = resolveRegistryComponent(meta, mode, cellTypes)
          if (regComp) return <div key={col.id}>{regComp({ value, onChange, ...(meta.config !== undefined ? { config: meta.config } : {}) })}</div>
        }
