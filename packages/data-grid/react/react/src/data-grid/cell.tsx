...
export function DataGridCell({ cell, row }: CellProps) {
  ...
  if (meta?.isSystemColumn) {
    if (columnId === SELECTION_COLUMN_ID) {
      ...
      const isIndeterminate =
        typeof row.getIsSomeSelected === 'function'
  ...
  const isEditingThisCell =
    editMode === 'cell' && editingState.editingCellId === cellId
    ...
    return (
      <Td style={pinStyles}>
        <Input
          autoFocus
          value={
            (editingState.editingValues[columnId] ?? '') as string | number | readonly string[]
          }
  ...
  // ── row editing mode ──────────────────────────────────────────────────────
  const isEditingRow =
    (editMode === 'row' || editMode === 'modal') &&
    editingState.editingRowId === row.id
  ...
  return (
    <Td style={pinStyles} onDoubleClick={handleDoubleClick}>
      {flexRender(
        ...
        cell.getContext() as unknown as Record<string, unknown>,
      )}
    </Td>
  )
}
