...
- ed0250e: `cell` takes the bare type id: `cell: 'date'` is `cell: { type: 'date' }`.
...
- 7131953: Rework how column-level defaults, `initialState` and controlled `state` interact.
...
- ed0250e: **Breaking:** `createColumnHelper` now spells every column slot the way `ColumnDef` spells it.
...
- ed0250e: Fix the table-level resizing gate and add `globalFiltering.manual`.
...
- ed0250e: Fix `ColumnDef.footer`'s JSDoc: it is not part of the default layout, but you do not read it
...
- ed0250e: Warn in development when `pageSize` is set twice.
...
- ed0250e: Warn in development when a column seed has no feature to undo it.
