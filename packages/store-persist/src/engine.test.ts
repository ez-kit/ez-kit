...
function bind(store: object, fields: FieldDescriptor[], options: PersistOptions = {}): PersistBinding {
	return createBinding(store, objectPort, fields, options)
}

const tick = (): Promise<void> => new Promise((resolve) => setTimeout(resolve, 0))

describe('@ez-kit/store-persist engine', () => {
	it('hydrates the store from the substrate on connect', () => {
		const fake = fakePort()
	...
	it('writes a store change once and terminates the loop', async () => {
		const fake = fakePort()
		const engine = createPersistEngine(fake.port)
		const store = observable({ q: '' })
		engine.connect(bind(store, [{ path: ['q'], parser: str }]))
	...
	it('coalesces changes from multiple bindings into one commit', async () => {
		const fake = fakePort()
		const engine = createPersistEngine(fake.port)
		const a = observable({ q: '' })
		const b = observable({ items: 0 })
