...
describe('@ez-kit/store-persist async hydration', () => {
	it('awaits an async get() and fills fields still at their default', async () => {
		const state = observable({ q: '' })
		const binding = createBinding(state, objectPort, [{ path: ['q'], parser: paramString() }], {})
