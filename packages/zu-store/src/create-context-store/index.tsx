...
export type CreateContextStoreFactory<
  TStore extends StoreApi<unknown>,
  TInitProps extends object,
> = (initProps: TInitProps) => TStore
...
function getStoreFromContext<TStore extends StoreApi<unknown>>(
  store: TStore | null,
): TStore {
  if (!store) {
...
>(
  ...
  function useShallowStore<TSelected>(
    selector: (state: ExtractState<TStore>) => TSelected,
  ): TSelected {
    const store = getStoreFromContext(useContext(StoreContext))
  ...
  function Item<TSelected>({
    selector,
    children,
  }: ItemProps<TStore, TSelected>): ReactElement {
