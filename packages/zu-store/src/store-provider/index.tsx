...
/**
 * App-level composition root for zu-store stores. Mounts the persist engines (one per source —
 * single writer, no races) and/or a store cache once at the top of the tree, so descendant
 * `persist()` capabilities can resolve the {@link PersistProvider}-published engines service and
 * cached stores share that one cache.
 *
 * Composition (outermost → innermost): `PersistProvider(persist) ∘ cache.Provider`. `PersistProvider`
 * publishes the engines as a store-core service (and extends the inherited registry), so a single
 * service scope exposes the engines to plugins whether a store is cached or not. Either layer is
 * skipped when its prop is omitted.
 *
 * The engines it mounts are manager-agnostic: one `StoreProvider` from either binding package can
 * serve Zustand and Valtio stores in the same tree, since a binding carries its own store port.
 */
