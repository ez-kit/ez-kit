	...
	for (const field of fields) {
		...
		for (const segment of path.slice(0, -1)) {
			if (parent === null || typeof parent !== 'object' || !(segment in parent)) {
				throw new Error(
					`[valtio-kit] search-params: path "${label}" is unreachable — "${segment}" is missing at bind time.`,
				)
			}
		...
		if (parent === null || typeof parent !== 'object') {
			throw new Error(`[valtio-kit] search-params: path "${label}" has no object parent at bind time.`)
		}
		...
		if (descriptor?.get && !descriptor.set) {
			throw new Error(
				`[valtio-kit] search-params: "${label}" is a getter-only property and cannot be written from the URL.`,
			)
		}
		...
		if (!isPlainData(leafValue)) {
			throw new Error(
				`[valtio-kit] search-params: "${label}" resolves to a class instance — sync its fields individually ` +
