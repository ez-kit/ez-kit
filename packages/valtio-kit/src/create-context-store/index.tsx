 ...
 * behavior unchanged: returns `{ Provider, useStore, useSnapshot, useContextStore, Item }` and creates
 * the proxy once per Provider via `useRef`. Read with `useStore()`/`useSnapshot()` (auto-tracked
 * snapshot), write through the raw proxy from `useContextStore()`. Pass `plugins` to bind capabilities
 * to the Provider's mount lifetime.
 */
