	...
	for (const field of fields) {
		...
		for (const segment of path.slice(0, -1)) {
			if (parent === null || typeof parent !== 'object' || !(segment in parent)) {
				throw new Error(`[va-store] persist: path "${label}" is unreachable — "${segment}" is missing at bind time.`)
			}
			parent = (parent as Record<string, unknown>)[segment]
		}

		if (parent === null || typeof parent !== 'object') {
			throw new Error(`[va-store] persist: path "${label}" has no object parent at bind time.`)
		}
		...
		if (descriptor?.get && !descriptor.set) {
			throw new Error(
				`[va-store] persist: "${label}" is a getter-only property and cannot be written from the substrate.`,
			)
		}
		...
		if (!isPlainData(leafValue)) {
			throw new Error(
				`[va-store] persist: "${label}" resolves to a class instance — sync its fields individually ` +
