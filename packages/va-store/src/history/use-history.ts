 ...
 */
export function useHistory<T extends object, TSlice extends object = T>(
	store: T & { history: StoreHistory<TSlice> },
): StoreHistory<TSlice> & HistorySnapshot<TSlice> & { canUndo: boolean; canRedo: boolean } {
	const { history } = store
	...
	const snap = useSnapshot(history.state) as unknown as HistorySnapshot<TSlice>
