 ...
 * what only React Aria can show — that each **chip** reads the label of a value the current
 * search never returned, and that picking a result adds a chip and empties the query.
 */
...
function chipTexts(container: HTMLElement): string[] {
	const chips = container.querySelector('[data-slot="selected-values"]')
	return chips === null ? [] : [...chips.children].map((chip) => chip.textContent)
}

describe('@ez-kit/form-heroui searchable multiselect', () => {
	...
	it('renders a combo box rather than the plain select trigger', () => {
		render(<SearchableCase cities={['lis']} />)
	...
	it('offers exactly the rows the source returned, without filtering them again', async () => {
		...
		// against the typed text — passing `items` is what turns that off.
		expect(await screen.findByRole('option', { name: 'Berlin' })).toBeInTheDocument()
		expect(screen.getByRole('option', { name: 'Bergen' })).toBeInTheDocument()
	})
	...
	it('adds a chip and clears the query when a result is picked', async () => {
		...
		await user.click(await screen.findByRole('option', { name: 'Bergen' }))
	...
	it('leaves a non-searchable multiselect on the plain select trigger', () => {
		function PlainCase() {
			return (
				<FormOptionSources value={SOURCES}>
