...
// After
createContextStore(() => withPersist(proxy(new Filters())))
```
...
capability name twice (`withHistory(withHistory(store))`) now throws instead of silently
double-registering.

`@ez-kit/va-store`'s `persist()` plugin is replaced by `withPersist(target, options?)`, a factory-
position wrapper that mutates and returns the proxy it's given: `withPersist(proxy({ … }), { fields })`.
It composes with the new `withHistory` capability by nesting — `withPersist(withHistory(proxy({ … })))`.
